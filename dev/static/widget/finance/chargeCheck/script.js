			        ...
			        if (result.status == 0) {
			        	if (saveType == 1) {
				        	if (result.data != null) {
								...
								common.msgs.new_msg("warning", "已审核完所有充值记录", window.location = './index.html?goto=finance&mold=list');
								;
				        	}
			        	} else {
...
function queryAccountChargeInfo () {
...
common.pluginsTools.loadPlugins(init, ['template']);
